...
; todo, 待研究1.8这道题可以用不动点做吗? 应该也是可以的, 然后分析下近似序列和之前的方法是不是一样? 感觉也是一样的...关键是迭代方程一样的话...
; 见ch1/cube_root folder 
; a = x^3 => x = a / x^2 
;     等价于求  => x = (a/x^2+2x) / 3 的不动点
; if x1是一个不动点, 
; x1^3 = a , a / x^2 = (a/x^2+2x) / 3  
; 左边等于x1, 右边等于(x1+2x1)/3 = x1. 所以根据同样的方法, 得证.
; 但是存在一个疑问? 这个优化是怎么找到的???(a/x^2+2x) / 3 见p100_newton_method.scm

; 所以不动点感觉可以应用在很多方面啊, 在数值计算方面...还是比较有用的